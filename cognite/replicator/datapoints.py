...
def retrieve_insert(
    i,
    src_ext_id_list_parts,
    dst_ext_id_list,
    CLIENT_SRC,
    CLIENT_DST,
    keep_asset_connection,
    num_threads,
):
    ...
    for src_ext_id in src_ext_id_list_parts[i]:
        if src_ext_id in dst_ext_id_list:
            ...
            logging.debug(
                f"Latest timestamp source with ext_id {src_ext_id}: {latest_src_dp[0].timestamp}"
            )
...
def replicate(CLIENT_SRC, CLIENT_DST, keep_asset_connection=True, num_threads=10):
    ...
    src_ext_id_list_parts = [
        src_ext_id_list[x : x + step] for x in range(0, len(src_ext_id_list), step)
    ]
    jobs = []
