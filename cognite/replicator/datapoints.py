    ...
    for src_ext_id in src_ext_id_list_parts:
        if src_ext_id in dst_ext_id_list:
            ...
            if not latest_src_dp:
                logging.info(f"Thread {thread_id}: No datapoints found in source -- skipping time series associated with: {src_ext_id}")
                continue

            logging.debug(f"Thread {thread_id}: Latest timestamp source with ext_id {src_ext_id}: {latest_src_dp[0].timestamp}")
            ...
            if not latest_destination_dp:
                latest_dst_time = 0
                logging.info(f"Thread {thread_id}: No datapoints in destination, starting copying from time(epoch): {latest_dst_time}")
...
def replicate(client_src, client_dst, keep_asset_connection=True, num_threads=10, limit=100000):
    ...
    for thread_id in range(num_threads):
        ...
        p = mp.Process(
            ...
            args=(thread_id, parts, dst_ext_id_list, client_src, client_dst, limit),
        )
        jobs.append(p)
        p.start()
    for p in jobs:
